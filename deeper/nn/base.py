...
class NNBase(object):
    ...
    def __init__(self, param_dims_dense, param_dims_sparse={}, **hypers):
        ...
        for p, v in hypers: setattr(self, p, v)
    ...
    def train_sgd(self, X, y,
        ...
        try:
            if verbose: print "Begin SGD..."
            for idx, alpha in itertools.izip(idxiter, alphaiter):
                if verbose and counter > 0 and counter % printevery == 0:
                    print "  Seen %d in %.02f s" % (counter, time.time() - t0)
                if verbose and counter > 0 and counter % costevery == 0:
                    if devidx != None:
